...
<div class="wrap sub_type2"> <!-- (내부)sub_type1, (외부)sub_type2 -->
  ...
  <div class="tab_version">
    ...
    <?php if ($usingPreviousVersion) {
      echo "<a href=\"". $outUrl ."/". $topPathPreviousVersion ."/android/index.php\"><span>". L::title_tab_previous_version ."</span></a>";
    }?>
  </div>

  <?php
    list($html_list, $pagenation, $currPage) = getPaginationSnippets("android", false);
  ?>
  ...
  <div class="container">
    ...
    <div class="tab_os">
      ...
      <?php } ?>
      <?php if ($usingiOS) { ?>
      <a href="../ios/index.php"><span>iOS</span></a>
      <?php } ?>
    </div>
    ...
    <div class="cont_area">
      ...
      <div class="item_area">
        ...
        if (isset($_POST['search']) && strlen($pattern) > 0 && $pattern != "Select") {
          echo getHtmlSnippets("android", false, true, $pattern, $html_list);
        } else {
          echo getHtmlSnippets("android", false, false, $pattern, $html_list);
        }
        ?>
        <!-- //item -->
      </div>
