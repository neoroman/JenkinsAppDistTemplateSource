...
if [ -f $OUTPUT_FOLDER/$UPLOAD_IPA_FILE ]; then
    ...
    # Extract metadata from the IPA file
    echo "$HOSTNAME > Extracting app metadata from IPA file..."
    # Create a temporary directory for IPA extraction
    TEMP_DIR=$(mktemp -d)
    unzip -q "$OUTPUT_FOLDER/$UPLOAD_IPA_FILE" -d "$TEMP_DIR"
    
    # Find the main app directory first, then get its Info.plist
    APP_DIR=$(find "$TEMP_DIR/Payload" -name "*.app" -type d -maxdepth 1 | head -1)
    INFO_PLIST="${APP_DIR}/Info.plist"
    if [ -f "$INFO_PLIST" ]; then
        BUNDLE_ID=$(/usr/libexec/PlistBuddy -c "Print :CFBundleIdentifier" "$INFO_PLIST")
        APP_VERSION_NUM=$(/usr/libexec/PlistBuddy -c "Print :CFBundleShortVersionString" "$INFO_PLIST")
        BUILD_NUM=$(/usr/libexec/PlistBuddy -c "Print :CFBundleVersion" "$INFO_PLIST")
        
        echo "$HOSTNAME > App Info - Bundle ID: ${BUNDLE_ID}, Version: ${APP_VERSION_NUM}, Build: ${BUILD_NUM}"
        
        # Validate IPA before uploading - this will check if the build already exists
        echo "$HOSTNAME > Validating IPA file to check for existing builds..."
        VALIDATE=$(xcrun altool --validate-app -f "$OUTPUT_FOLDER/$UPLOAD_IPA_FILE" -t ios -u "${APPSTORE_AGENT_EMAIL}" -p "${APPSTORE_AGENT_PASSWORD}" 2>&1)
        
        if [[ "$VALIDATE" == *"No errors validating"* ]]; then
            echo "$HOSTNAME > IPA validation successful, no duplicate builds found."
        elif [[ "$VALIDATE" == *"bundle version must be higher"* || "$VALIDATE" == *"value that has already been used"* ]]; then
            echo "$HOSTNAME > WARNING: A build with this version already exists in App Store Connect!"
            echo "$HOSTNAME > Validation error: $(echo "$VALIDATE" | grep "bundle version must be higher")"
            
            # Display relevant app info
            echo "$HOSTNAME > Existing app details:"
            echo "Bundle ID: ${BUNDLE_ID}"
            echo "Marketing Version: ${APP_VERSION_NUM}"
            echo "Build Number: ${BUILD_NUM}"
            
            echo "$HOSTNAME > Upload canceled to prevent TMS-90189 error."
            echo "$HOSTNAME > Please increment the build number and try again."
            rm -rf "$TEMP_DIR"
            exit 1
        else
            echo "$HOSTNAME > Validation warning/error: ${VALIDATE}"
            echo "$HOSTNAME > This doesn't appear to be a duplicate build issue. Proceeding with upload..."
        fi
    else
        echo "$HOSTNAME > Warning: Could not find Info.plist in IPA to extract metadata"
    fi
    ...
    # Upload to App Store
    echo "$HOSTNAME > Uploading IPA to App Store (${SIZE_UPLOAD_IPA_FILE})..."
    echo "$HOSTNAME > This process may take several minutes depending on your connection and file size."

    if [ "${DRY_RUN:-0}" -eq 1 ]; then
        echo "$HOSTNAME > DRY RUN: Skipping actual upload."
        echo "$HOSTNAME > Would have executed: xcrun altool --upload-app -t ios -f \"$OUTPUT_FOLDER/$UPLOAD_IPA_FILE\" -u \"${APPSTORE_AGENT_EMAIL}\" -p \"${APPSTORE_AGENT_PASSWORD}\""
    else
        ERROR=$(xcrun altool --upload-app -t ios -f "$OUTPUT_FOLDER/$UPLOAD_IPA_FILE" -u "${APPSTORE_AGENT_EMAIL}" -p "${APPSTORE_AGENT_PASSWORD}" 2>&1)
        if [[ "$ERROR" == *"No errors uploading"* ]]; then
            echo "$HOSTNAME > Upload successful. App is being processed by App Store Connect."
        elif [ "$ERROR" != "" ]; then
